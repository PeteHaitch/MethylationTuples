  ...
  if (!is.na(m)) {
    ...
    if (is.null(an) || !("counts" %in% an)) {
      ...
      if (any(is.na(match(.makeMethPatNames(m),
        ...
        # TODO: Need to determine how expensive it is to test this on 
        # HDF5Array- and DSArray-backed objects  
...
# TODO: Some sort of validity check on 'counts? E.g., they should be positive 
...
setMethod("MethPat", "ANY", 
          function(assays, rowTuples, ...) {
            ...
            if (missing(rowTuples)) {
              stop("Must supply an 'MTuples' object as the 'rowTuples' ", 
                   "argument")
            }
...
# TODO(?): methLevel,MethPat-method, getCoverage,MethPat-method (depending on 
#          what I call the generic in R/AllGenerics.R)
...
#' @rdname MethPat-class
#' @param object A MethPat object.
#' @importFrom methods callNextMethod setMethod
#' @export
setMethod("show", "MethPat", 
          function(object) {
            callNextMethod() # nocov start
            margin <- ""
            cat(margin, "size: ", size(object), "\n", sep = "")
