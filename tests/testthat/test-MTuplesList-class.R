...
test_that("MTuplesList() returns a valid object with no arguments", {
  expect_true(validObject(MTuplesList()))
})

test_that("MTuplesList() returns a valid object when size = 0", {
  expect_true(validObject(mtl0))
})

test_that("MTuplesList() returns a valid object when size = 1", {
  expect_true(validObject(mtl1))
})

test_that("MTuplesList() returns a valid object when size = 2", {
  expect_true(validObject(mtl2))
})

test_that("MTuplesList() returns a valid object when size >= 3", {
  expect_true(validObject(mtl3))
})

test_that("MTuplesList() works for list input", {
  expect_true(validObject(MTuplesList(list(mt1))))
  expect_true(validObject(MTuplesList(list(mt1, mt1))))
  expect_true(validObject(MTuplesList(list(mt1, mt1, mt1))))
})

test_that("MTuplesList() works for >= 1 MTuples input", {
...
test_that("MTuplesList constructor returns errors on bad input", {
  expect_error(MTuplesList(mt1, mt2), 
               "all 'MTuples' objects in '...' must have the same 'size'")
  gr <- granges(gt2)
...
###
context("relistToClass,MTuplesList-method workds")
...
### apply methods
###
context("endoapply,MTuplesList-method works")
