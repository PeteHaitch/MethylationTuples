...
### read.methtuple
###
context("read.methtuple")

test_that("Returns MethPat object on good input", {
  expect_is(suppressMessages(
    read.methtuple('fake_data/good_fake_1.tsv', 
                   seqinfo = seqinfo)), 
    "MethPat")
  expect_is(suppressMessages(
    read.methtuple('fake_data/good_fake_3.tsv', 
                   seqinfo = seqinfo)), 
    "MethPat")
})
...
test_that("Returns warning on bad input", {
  ...
  expect_warning(suppressMessages(
    read.methtuple('fake_data/bad_fake_1.tsv', 
  ...
  expect_warning(suppressMessages(
    read.methtuple('fake_data/bad_fake_3.tsv', 
                   seqinfo = seqinfo)), 
    paste0("Some tuples are stranded \\('strand' = '\\+' or '-'\\) and some ", 
           "are unstranded \\('strand' = '\\*'\\)"))
})
...
test_that("sample_names are checked", {
  expect_error(
    suppressMessages(read.methtuple(
...
test_that("tuple sizes are checked", {
  expect_error(suppressMessages(read.methtuple(
...
test_that("seqinfo is sanity checked", {
  expect_warning(
    suppressMessages(
      z <- read.methtuple('fake_data/good_fake_1.tsv', 
